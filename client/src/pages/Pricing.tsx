      ...
      <main className="p-6 lg:p-10 max-w-5xl mx-auto">
        <header className="text-center mb-16">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-foreground tracking-tighter uppercase italic mb-4">
            Simple Pricing. <span className="text-primary">No Hype.</span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
          ...
          {/* Pro Plan */}
          <Card className="bg-card border-primary/30 shadow-xl shadow-primary/5 relative overflow-hidden group scale-105">
            <div className="absolute top-0 right-0 bg-primary text-primary-foreground text-[10px] font-black px-3 py-1 uppercase tracking-widest rounded-bl-lg">
              Recommended
            </div>
            <CardContent className="p-8">
              <div className="mb-8">
                <h3 className="text-xl font-bold text-primary mb-2 uppercase tracking-widest">Trader Pro</h3>
                <div className="flex items-baseline gap-1">
                ...
                <p className="text-[10px] text-primary/70 font-bold uppercase tracking-widest mt-2">or $190/year (2 months free)</p>
              </div>

              <div className="space-y-4 mb-10">
                <div className="text-[10px] font-black text-primary/50 uppercase tracking-widest mb-2">Everything in Free, plus:</div>
                {features.filter(f => !f.free).map((f, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <Check size={16} className="text-primary flex-shrink-0" />
              ...
              {isPro ? (
                <Button 
                  ...
                  className="w-full h-14 bg-primary hover:bg-primary/90 text-primary-foreground font-black uppercase tracking-[0.15em] text-xs shadow-lg shadow-primary/20"
                >
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="p-6 text-xs font-black text-primary uppercase tracking-widest text-center">Pro</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/50">
                {features.map((f, i) => (
                  <tr key={i} className="hover:bg-muted/10 transition-colors">
                    <td className="p-6 text-sm text-foreground/80 font-medium">{f.name}</td>
                    <td className="p-6 text-center">
                      {f.free ? <Check size={18} className="text-primary mx-auto" /> : <X size={18} className="text-muted-foreground/30 mx-auto" />}
                    </td>
                    <td className="p-6 text-center">
                      {f.pro ? <Check size={18} className="text-primary mx-auto" /> : <X size={18} className="text-muted-foreground/30 mx-auto" />}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20">
          {[
            ...
            <div key={i} className="bg-card border border-border p-6 rounded-2xl text-center shadow-sm">
        ...
        <div className="text-center pb-20">
          ...
          {isPro ? (
            <Button 
              ...
              className="h-14 px-10 bg-primary hover:bg-primary/90 text-primary-foreground font-black uppercase tracking-[0.15em] text-xs shadow-lg shadow-primary/20"
            >
              Manage Your Account
            </Button>
          ) : (
