...
import { api, buildUrl, type TradeInput, type TradeResponse } from "@shared/routes";
...
export function useCreateTrade() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Trade Logged",
        description: "Your trade has been successfully recorded in the journal.",
      });
    },
    onError: (error) => {
      toast({
...
export function useUpdateTrade() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...updates }: { id: number } & Partial<TradeInput>) => {
