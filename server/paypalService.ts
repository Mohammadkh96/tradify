...
const ALL_PLAN_IDS = new Set([
  ...
  PAYPAL_FM_COACH_PLAN_ID,
  PAYPAL_FM_COACH_ANNUAL_PLAN_ID,
].filter(Boolean));

function getPlanKeyFromId(planId: string): { tier: PlanTier; period: BillingPeriod } | null {
  if (planId === cachedPlanIds.COACH_annual || planId === PAYPAL_COACH_ANNUAL_PLAN_ID || planId === PAYPAL_FM_COACH_ANNUAL_PLAN_ID) return { tier: 'COACH', period: 'annual' };
  if (planId === cachedPlanIds.COACH_monthly || planId === PAYPAL_COACH_PLAN_ID || planId === PAYPAL_FM_COACH_PLAN_ID) return { tier: 'COACH', period: 'monthly' };
