...
export async function registerRoutes(
  ...
  // Stripe Billing API (uses Replit Stripe integration)
  app.get("/api/billing/products", async (req, res) => {
    try {
      const { getStripeSync } = await import('./stripeClient');
      const stripeSync = await getStripeSync();
      
      // Query products with prices from stripe schema
  ...
  app.post("/api/billing/portal", requireAuth, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to open billing portal", error: error.message });
    }
  });
