...
- **AI Marketing Hub:** Admin-only command center with a Marketing Dashboard, AI-powered Content Studio (Social Posts, Reel Scripts, Blog Articles, Email Campaigns), Meta Ads Strategist, Brand Settings, Content Library, and **Content Factory** (funnel-based asset generation). It uses real user data for content generation and includes a deduplication system.
- **Content Factory (Funnel Generator):** AIDA marketing funnel engine (`server/funnel-generator.ts`) generating stage-aware marketing assets. 5 stages: Awareness (TOFU), Consideration (MOFU), Decision (BOFU), Action, Loyalty. Each stage has distinct content types, prompt strategies, and tone. Generates ad images (gpt-image-1), video reels (9:16 MP4 via ffmpeg with AI key-frames + zoompan effect, configurable 4/6/8s duration), stock photos (photography-style AI images), social posts, reel scripts, ad copy, blog articles, email campaigns, landing pages (HTML), comparison posts, testimonial posts, and case studies. Assets downloadable individually or as ZIP bundle (`archiver`) with organized folders (images/, videos/, stock/, text/, articles/, emails/, pages/). Frontend: `client/src/pages/admin/FunnelGenerator.tsx` with video player preview and duration selector. Routes: `/api/admin/marketing/funnel/*`.
