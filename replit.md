...
- **Activation: Sample-mode dashboard.** When a user is logged in but has no real trades and no live MT5 connection, the Dashboard and Prop Firm Tracker render deterministic sample data (60 days of trades, equity curve, FTMO $50K sample challenge, behavioural insight) so the product feels alive immediately. Implemented in `client/src/lib/sampleData.ts`, gated by `client/src/hooks/useSampleMode.ts` (waits for queries to resolve before activating; dismissible via `tradify_sample_mode_dismissed` localStorage). Account switch/delete UI is hidden in sample mode to prevent acting on synthetic accounts.
- **Activation: 6-step MT5 onboarding wizard at `/mt5-bridge`.** Intro → Token → Download → Run → Verify → Done, with stepper UI, OS-specific (Windows/Mac) instructions, troubleshooting expandables, and live status polling that auto-advances to "Done" when the connector first reports CONNECTED. The connector script in `client/src/lib/mt5ConnectorScript.ts` is templated with `__TRADIFY_USER_ID__`, `__TRADIFY_SYNC_TOKEN__`, `__TRADIFY_API_URL__` markers and substituted at download time.
- **Activation: First-sync "aha moment" modal** (`client/src/components/FirstSyncMoment.tsx`, mounted in `MainLayout`). Fires once per user when MT5 status is observed CONNECTED without a prior `tradify_first_sync_seen_<userId>` localStorage flag.
