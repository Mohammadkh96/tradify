...
- Tiered subscription model (Free, Pro, Elite) with feature gating, billed via either PayPal or Stripe (parallel processors). Stripe checkout uses lookup_key prices seeded by `scripts/seed-stripe-products.ts`; webhook at `/api/stripe/webhook` (registered before `express.json()` so signature verification gets raw body) handles `checkout.session.completed` (grant tier) and `customer.subscription.deleted` (downgrade to FREE only when user has no PayPal sub and provider is stripe).
- Per-trade chart upload: `chart_url` column on `trade_journal`; routes `POST/GET/DELETE /api/trades/:id/chart` (multer 5MB png/jpg/webp, ownership-checked, served via auth-checked proxy from private object-storage bucket); `TradeChartUploader.tsx` thumbnail+lightbox component on every Journal row.
- Weekly performance digest email: server/weeklyDigest.ts runs every 30 min, sends per-user on local Sunday 9am (env `WEEKLY_DIGEST_LOCAL_HOUR`); 7-day stat grid + insight; opt-out via `alert_preferences.weekly_digest_enabled`; atomic dedupe via partial unique index `uq_notifications_dedupe(user_id, type, dedupe_key)` claimed BEFORE email send to prevent double-send.
