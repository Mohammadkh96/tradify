...
- **Database Backups:** Daily Neon snapshot pipeline. `server/backup-service.ts` runs `pg_dump` (auto-resolves a server-version-matching binary from `/nix/store`, configurable via `PG_DUMP_BIN`) → gzip → uploads to Replit Object Storage at `db-backups/YYYY/MM/tradify-*.sql.gz`. Scheduler armed in `server/index.ts` runs at 03:30 UTC daily; idempotent CLI at `scripts/backup-db.ts` (`npx tsx scripts/backup-db.ts`). Retention: last 30 daily + last 12 monthly (first successful backup of each calendar month is flagged `is_monthly`). Failures email `ADMIN_EMAIL` via `emailService.sendBackupFailureAlertEmail`; ≥2 consecutive failures escalate the subject. Admin observability at `/admin/backups` (last run, status, size, duration, latest error, recent runs — no download/restore UI by design). Endpoints: `GET /api/admin/backups/status`, `POST /api/admin/backups/run`. DB table: `database_backups`. Restore procedure: `docs/BACKUP_RESTORE.md`.